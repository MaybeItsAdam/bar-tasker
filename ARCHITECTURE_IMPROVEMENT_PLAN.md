...
- ~~Forwarding properties on `AppCoordinator` are deleted; views read from the relevant service directly.~~ **Method forwarders culled (step 3.6); property cull in progress (step 3.7).** Deleted so far: `rootScopeFocusLevel`, `currentSiblingIndex`, `currentParentId` (all `NavigationState`-owned). Remaining forwarders (`tasks`, `listId`, `errorMessage`, `hideFuture`, `showChildrenInMenus`, `rootTaskView`, `selectedRootDueBucketRawValue`, `selectedRootTag`, `username`, `remoteKey`, `availableLists`, `isLoading`, `taskEisenhowerLevels`, `cache`, `activeCredentials`). The real architectural win — views taking `repository`/`navigationState`/services as separate `@Environment` objects — is being done one property at a time rather than as a rename (`manager.X` → `manager.subobj.X` still routes through the god object). The persistence-side-effect gate on the three `TaskListViewModel` setters is now cleared (see step 3.7), so `rootTaskView`/`selectedRootDueBucketRawValue`/`selectedRootTag` are unblocked — they only need `TaskListViewModel` injected into the popover environment (the same move already done for `NavigationState`).
...
- **`KanbanTaskDataSource` decoupled from `AppCoordinator` (step 3.7a — the unblocker).** `AppCoordinator` no longer conforms to `KanbanTaskDataSource`; a dedicated `KanbanTaskDataSourceAdapter` (held strongly by `AppCoordinator` because `KanbanManager.dataSource` is `weak`) implements the protocol against `repository`/`navigationState`/`taskListViewModel` directly. `KanbanManager`'s body is unchanged. This is what let `currentSiblingIndex`/`currentParentId` (both protocol members) be deleted; `tasks`/`hideFuture`/`rootTaskView`/`cache` are now likewise deletable.
- **View-shaping persistence moved into `TaskListViewModel` (step 3.7b — the gate-clearer).** `rootTaskView`/`selectedRootDueBucketRawValue`/`selectedRootTag` now persist from TLVM's own `didSet`s (TLVM took a `PreferencesStore` dep and loads their initial values in `init`, where `didSet` doesn't fire). The `AppCoordinator` forwarders for them are now pure pass-throughs, so they're unblocked for deletion once `TaskListViewModel` is injected into the popover environment.
