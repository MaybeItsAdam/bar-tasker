...
struct DayBoundary: Equatable, Sendable {
  ...
  static let defaultRolloverHour = 4

  let rolloverHour: Int
  let calendar: Calendar

  init(rolloverHour: Int = DayBoundary.defaultRolloverHour, calendar: Calendar = .current) {
  ...
  func day(offsetBy offset: Int, from date: Date) -> Date {
  ...
  /// `date`, oldest first.
  func days(endingOn date: Date, count: Int) -> [Date] {
  ...
  /// `date`, oldest first.
  func weeks(endingOn date: Date, count: Int) -> [Date] {
    guard count > 0 else { return [] }
