...
struct Daily: Codable, Equatable, Identifiable, Sendable {
  ...
  static let intervalRange = 1...366
  ...
  static func clampInterval(_ days: Int) -> Int {
  ...
  /// annotation.
  var scheduleLabel: String { Daily.scheduleLabel(for: schedule) }
  ...
  static func scheduleLabel(for schedule: Schedule) -> String {
    switch schedule {
  ...
  mutating func setSchedule(_ schedule: Schedule, anchor: Date = Date()) {
    switch schedule {
    case .weekdays(let days):
