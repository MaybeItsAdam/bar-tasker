...
enum DayLogAggregator {
  ...
  struct DaySummary: Equatable, Sendable {
    let key: String
    let day: Date
    let completed: [DayLogEvent]
    let plannedTaskIds: [Int]
    let unfinishedTaskIds: [Int]
    let deferredTaskIds: [Int]
    let invalidatedTaskIds: [Int]
    ...
    let completedDailyIds: Set<String>

    var completedCount: Int { completed.count }
    var plannedCount: Int { plannedTaskIds.count }
    var unfinishedCount: Int { unfinishedTaskIds.count }

    static func empty(key: String, day: Date) -> DaySummary {
      DaySummary(
        key: key,
        day: day,
        completed: [],
  ...
  static func netCompletions(_ events: [DayLogEvent]) -> [DayLogEvent] {
  ...
  static func recordedDayCount(events: [DayLogEvent], boundary: DayBoundary) -> Int {
  ...
  static func firstRecordedDay(events: [DayLogEvent], boundary: DayBoundary) -> Date? {
