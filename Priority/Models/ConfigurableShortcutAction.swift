...
enum ConfigurableShortcutAction: String, CaseIterable, Identifiable {
  ...
  var defaultBinding: String {
    ...
    // Every unmodified letter is already spoken for by the existing tabs,
    // filters, and sequence starters, so this one takes a shifted key.
    // `shift+t` reads as "Today", which is what the view answers.
    case .rootTabDaily: return "shift+t"
    }
  }
}
