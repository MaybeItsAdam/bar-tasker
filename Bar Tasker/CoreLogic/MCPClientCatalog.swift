...
/// How Bar Tasker can add itself to a given MCP client.
///
...
enum MCPClientInstallStyle: Equatable {
  /// A plain JSON file that only ever holds MCP configuration. Bar Tasker can
...
/// A known MCP client and everything needed to add Bar Tasker to it.
...
enum MCPClientCatalog {
  /// The server name Bar Tasker registers itself under in every client.
  static let serverName = "bar-tasker"
